...
### Surfaces
- **Home** (`app/page.tsx`) — 5 sections : Hero ("Score your hook in 5 seconds. Fix it in one click." + **deux chemins persona côte à côte** : "✦ I have a hook → Score it" pointe `/analyzer` ; "🔬 I have a niche, no idea → Discover trends" pointe `/trends`) · How-it-works (4 cartes YOU DO/YOU GET) · `HomeAnalyzeWidget` (paste→score réel inline via `/api/analyze`) · Email capture · Final CTA. Pas de FAQ, pas de typing demo, pas de Features grid, pas de testimonials.
