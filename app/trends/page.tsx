...
interface Trend { title: string; channel: string; }
...
export default function TrendsPage() {
  ...
  const load = useCallback(async (slug: string) => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/trends${slug ? `?niche=${slug}` : ""}`);
        ...
        <div className="page-wrap">
          ...
          {!configured && (
            <div style={{ background: "rgba(255,184,0,.06)", border: "1px solid rgba(255,184,0,.3)", color: "var(--gold)", borderRadius: "var(--r2)", padding: "1.25rem 1.5rem", fontSize: ".85rem", lineHeight: 1.7 }}>
              Trends are not configured yet. Add a <strong>YOUTUBE_API_KEY</strong> to <code>.env.local</code> to pull real trending topics from YouTube.
            </div>
          )}
          ...
          {configured && !loading && !error && trends.length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {trends.map((t, i) => (
                <Link
                  key={i}
                  href={`/generator?topic=${encodeURIComponent(t.title)}${niche ? `&niche=${niche}` : ""}`}
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {t.channel && <div style={{ fontSize: ".72rem", color: "var(--muted)" }}>{t.channel}</div>}
                  </div>
          ...
          {configured && !loading && !error && trends.length === 0 && (
            <div style={{ textAlign: "center", padding: "4rem 2rem", color: "var(--muted)", fontSize: ".9rem" }}>
              No trends found for this niche right now.
            </div>
          )}
        </div>
      </div>
