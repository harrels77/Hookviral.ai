  ...
  return (
    <PageShell>
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        >
          <Icon name="arrow-left" /> Back to trends
        </Link>
          ...
          <div style={{ flex: 1, minWidth: "260px" }}>
            ...
            {niche && (
              <div style={{ marginTop: ".5rem", fontSize: ".78rem", color: "var(--muted)", fontFamily: "var(--fb)" }}>
                For: <span style={{ color: "var(--accent)" }}>{niche}</span> creators
              </div>
            )}
          </div>
          {data && state === "done" && (
            <span style={{ fontSize: "var(--text-xs)", fontFamily: "var(--fd)", fontWeight: 700, padding: "5px 14px", borderRadius: "var(--r-pill)", color: data.kind === "news" ? "var(--accent)" : "var(--success)", background: data.kind === "news" ? "var(--accent-soft)" : "var(--success-soft)", whiteSpace: "nowrap" }}>
              {data.kind === "news" ? "News spike — post today" : "Evergreen — build a series"}
            </span>
          )}
        </div>
      </div>
...
function ErrorState({ err, retry, hasTrend }: { err: string; retry: () => void; hasTrend: boolean }) {
  return (
    <div style={{ background: "var(--danger-soft)", border: "1px solid var(--danger)", borderRadius: "var(--r-lg)", padding: "2rem", textAlign: "center" }}>
      <div style={{ fontSize: "1rem", color: "var(--danger)", fontFamily: "var(--fb)", fontWeight: 500, marginBottom: ".5rem" }}>
        {err}
      </div>
      ...
      <div className="research-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0, 0.42fr) minmax(0, 0.58fr)", gap: "24px", alignItems: "start" }}>
        ...
        {hasContext ? (
          <div style={{ background: "var(--s1)", border: "1px solid var(--border)", borderRadius: "var(--r3)", padding: "1.5rem 1.6rem", position: "sticky", top: "1.5rem" }}>
            <div style={{ fontSize: ".66rem", color: "var(--accent)", textTransform: "uppercase", letterSpacing: "2px", fontFamily: "var(--fd)", fontWeight: 700, marginBottom: "1.25rem", display: "flex", alignItems: "center", gap: "8px" }}>
              <span style={{ width: "20px", height: "1px", background: "var(--accent)" }} /> Context
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "1.1rem" }}>
              {([
                ...
                ["Stakes", data.context.stakes],
                ["When", data.context.timeline],
              ] as [string, string][]).filter(([, v]) => v).map(([label, body]) => (
                <div key={label}>
                  <div className="kicker" style={{ fontSize: ".7rem", marginBottom: ".4rem" }}>
                    {label}
                  </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          <div style={{ fontSize: ".66rem", color: "var(--accent)", textTransform: "uppercase", letterSpacing: "2px", fontFamily: "var(--fd)", fontWeight: 700, marginBottom: ".25rem", display: "flex", alignItems: "center", gap: "8px" }}>
            <span style={{ width: "20px", height: "1px", background: "var(--accent)" }} /> {data.angles.length} angle{data.angles.length === 1 ? "" : "s"} to film
          </div>
          {data.angles.map((a, i) => (
...
function AngleCard({ a, i, niche }: { a: TrendAngle; i: number; niche: string }) {
  ...
  return (
    <div style={{ background: "var(--s1)", border: "1px solid var(--border)", borderRadius: "var(--r3)", padding: "1.25rem 1.4rem", display: "flex", flexDirection: "column", gap: "12px", transition: "border-color .2s" }}>
      ...
      {a.reasoning && (
        <div style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)", lineHeight: 1.6, fontStyle: "italic" }}>
          {a.reasoning}
        </div>
      )}
      ...
      <div style={{ background: "var(--s2)", border: "1px solid var(--border)", borderRadius: "var(--r2)", padding: ".85rem 1rem", marginTop: "4px" }}>
        <div className="kicker" style={{ fontSize: ".7rem", color: "var(--success)", marginBottom: ".4rem" }}>
          Ready hook
        </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
          {a.patternsUsed.map(p => (
            <Link key={p} href={patternHref(p)} title="Learn this pattern"
              style={{ fontSize: "var(--text-xs)", padding: "3px 9px", borderRadius: "var(--r-pill)", background: "var(--success-soft)", color: "var(--success)", border: "1px solid var(--success)", textDecoration: "none", fontFamily: "var(--fb)" }}>
              {p}
            </Link>
          ))}
        </div>
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginTop: "4px" }}>
        <button
          onClick={copyHook}
          style={{ display: "inline-flex", alignItems: "center", gap: "4px", padding: "7px 14px", borderRadius: "var(--r-pill)", border: "1px solid var(--border-strong)", background: "transparent", color: copied ? "var(--success)" : "var(--text-soft)", fontSize: "var(--text-xs)", cursor: "pointer", fontFamily: "var(--fb)" }}
        >
          {copied ? <><Icon name="check" /> Copied</> : <><Icon name="copy" /> Copy hook</>}
        </button>
        <Link
          href={`/generator?topic=${encodeURIComponent(a.angle)}${nicheQs}`}
          style={{ display: "inline-flex", alignItems: "center", gap: "4px", padding: "7px 14px", borderRadius: "var(--r-pill)", border: "1px solid var(--border-strong)", color: "var(--text-soft)", fontSize: "var(--text-xs)", textDecoration: "none", fontFamily: "var(--fb)" }}
        >
          <Icon name="zap" /> More hooks
        </Link>
        <Link
          href={`/analyzer?hook=${encodeURIComponent(a.hook)}`}
          style={{ display: "inline-flex", alignItems: "center", gap: "4px", padding: "7px 14px", borderRadius: "var(--r-pill)", border: "1px solid var(--accent)", color: "var(--accent)", fontSize: "var(--text-xs)", textDecoration: "none", fontFamily: "var(--fb)" }}
        >
        ...
        <button
          onClick={buildBrief}
          disabled={bs === "loading"}
          style={{ display: "inline-flex", alignItems: "center", gap: "4px", padding: "7px 14px", borderRadius: "var(--r-pill)", border: "1px solid var(--warning)", background: bs === "done" ? "var(--warning-soft)" : "transparent", color: "var(--warning)", fontSize: "var(--text-xs)", cursor: bs === "loading" ? "wait" : "pointer", fontFamily: "var(--fb)" }}
        >
          <Icon name="clipboard" /> {bs === "loading" ? "Building…" : bs === "done" ? "Hide brief" : "Faceless brief"}
        </button>
      </div>
      ...
      {bs === "error" && (
        <div style={{ fontSize: "var(--text-xs)", color: "var(--danger)" }}>
          Couldn&apos;t build the brief. <button onClick={buildBrief} style={{ background: "none", border: "none", color: "var(--accent)", cursor: "pointer", textDecoration: "underline", fontFamily: "var(--fb)", fontSize: "var(--text-xs)" }}>Retry</button>
        </div>
      )}
      {bs === "done" && brief && (
        <div style={{ background: "var(--s2)", border: "1px solid var(--border)", borderRadius: "var(--r2)", padding: "1rem 1.1rem", display: "flex", flexDirection: "column", gap: "10px", animation: "cardIn .3s ease" }}>
          {brief.voiceover.map((vo, k) => (
            <div key={k} style={{ display: "flex", flexDirection: "column", gap: "4px", paddingBottom: "8px", borderBottom: k < brief.voiceover.length - 1 ? "1px solid var(--border)" : "none" }}>
              ...
              <div style={{ fontSize: "var(--text-sm)", color: "var(--text)", lineHeight: 1.5 }}>{vo}</div>
              {brief.broll[k] && <div style={{ fontSize: "var(--text-xs)", color: "var(--text-soft)", lineHeight: 1.5 }}><strong style={{ fontWeight: 600 }}>B-roll:</strong> {brief.broll[k]}</div>}
              {brief.onScreenText[k] && <div style={{ fontSize: "var(--text-xs)", color: "var(--text-soft)", lineHeight: 1.5 }}><strong style={{ fontWeight: 600 }}>On-screen:</strong> {brief.onScreenText[k]}</div>}
            </div>
          ))}
          {brief.cta && (
            <div style={{ fontSize: "var(--text-sm)", color: "var(--success)", fontWeight: 500 }}><strong style={{ fontWeight: 600 }}>CTA:</strong> {brief.cta}</div>
          )}
        </div>
      )}
    </div>
