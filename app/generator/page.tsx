...
export default function GeneratorPage() {
  ...
  function spendCredit() {
    ...
    setCredits(n);
    const raw = localStorage.getItem("hv_credits");
    const s = raw ? JSON.parse(raw) : { resetAt: getMidnight() };
  ...
  function toggleFav(hook: Hook) {
    ...
    localStorage.setItem("hv_favs", JSON.stringify(updated));
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ topic, platforms, tone, niche: activeNiche, goal }),
      });
  ...
  async function generateScript(hook: Hook) {
    ...
    } catch { setScript({ hook: hook.text, bridge: "Could not generate script. Check your API key.", cta: "" }); }
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <div style={{ borderBottom: "1px solid var(--border)", padding: "2.5rem 1.5rem 2rem", textAlign: "center" }}>
          ...
          <p style={{ color: "var(--soft)", fontWeight: 300, fontSize: ".95rem" }}>Describe your content. Get 8 scored viral hooks. Copy. Post. Win.</p>
        </div>
        ...
        <div className="page-wrap">
          ...
          <div style={{ background: "var(--s1)", border: "1px solid var(--border)", borderRadius: "var(--r2)", padding: "1rem 1.25rem", marginBottom: "12px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px", flexWrap: "wrap", gap: "6px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                ...
                {credits <= 6 && <button onClick={() => setShowModal(true)} style={{ padding: "4px 12px", borderRadius: "100px", border: "1px solid rgba(255,45,107,.35)", background: "rgba(255,45,107,.06)", color: "var(--hot)", fontSize: ".72rem", cursor: "pointer", fontFamily: "var(--fb)" }}>Pro = unlimited →</button>}
              </div>
            </div>
            ...
            <div style={{ marginTop: "6px", fontSize: ".7rem", color: "var(--muted)", display: "flex", justifyContent: "space-between" }}>
              ...
              <span style={{ color: "rgba(108,58,255,.5)" }}>Pro: ∞ unlimited</span>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
            <Panel label="Platform">
              {PLATFORMS.map(p => <Chip key={p} label={p} active={platforms.includes(p)} onClick={() => togglePlatform(p)} color="plat" />)}
            </Panel>
            <Panel label="Tone">
              {TONES.map(t => <Chip key={t} label={t} active={tone === t} onClick={() => setTone(t)} color="tone" />)}
            </Panel>
          </div>
          ...
          <Panel label="Niche (optional)" style={{ marginBottom: "12px" }}>
            ...
            <button onClick={() => { setShowCustom(p => !p); setNiche(""); }}
              style={{ padding: "6px 14px", borderRadius: "100px", border: `1px solid ${showCustom ? "rgba(255,184,0,.5)" : "var(--border2)"}`, background: showCustom ? "rgba(255,184,0,.08)" : "transparent", color: showCustom ? "var(--gold)" : "var(--muted)", fontSize: ".8rem", cursor: "pointer", fontFamily: "var(--fb)", transition: "all .2s" }}>
              ✏ Other...
            </button>
            {showCustom && (
              <input ref={customRef} value={customNiche} onChange={e => setCustomNiche(e.target.value)} placeholder="Type your niche..."
                style={{ padding: "6px 14px", borderRadius: "100px", border: "1px solid rgba(255,184,0,.4)", background: "rgba(255,184,0,.04)", color: "var(--text)", fontSize: ".8rem", fontFamily: "var(--fb)", outline: "none", width: "160px", caretColor: "var(--gold)" }} />
            )}
          </Panel>
          ...
          {/* Generate */}
          <div style={{ display: "flex", gap: "12px", background: "var(--s1)", border: "1px solid var(--border)", borderRadius: "var(--r2)", padding: "1rem 1.25rem", marginBottom: "12px" }}>
            <button onClick={generate} disabled={loading}
              style={{ display: "flex", alignItems: "center", gap: "8px", flex: 1, justifyContent: "center", padding: "13px 28px", borderRadius: "100px", border: "none", background: loading ? "var(--s3)" : "linear-gradient(135deg,var(--hot),var(--electric))", color: "#fff", fontSize: ".95rem", fontWeight: 500, fontFamily: "var(--fb)", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? .7 : 1, transition: "all .3s" }}>
              {loading ? <><div style={{ width: "16px", height: "16px", borderRadius: "50%", border: "2px solid rgba(255,255,255,.3)", borderTopColor: "#fff", animation: "spin 1s linear infinite" }} />Generating...</> : <>⚡ Generate 8 Hooks</>}
            </button>
          </div>

          {error && <div style={{ marginBottom: "1rem", padding: "1rem 1.25rem", background: "rgba(255,45,107,.06)", border: "1px solid rgba(255,45,107,.2)", borderRadius: "var(--r2)", color: "var(--hot)", fontSize: ".875rem" }}>⚠ {error}</div>}

          {loading && (
            <div style={{ textAlign: "center", padding: "5rem 2rem" }}>
              <div style={{ width: "56px", height: "56px", borderRadius: "50%", border: "2px solid var(--border2)", borderTopColor: "var(--hot)", borderRightColor: "var(--electric)", animation: "spin 1s linear infinite", margin: "0 auto 1.5rem" }} />
              <div style={{ fontFamily: "var(--fd)", fontSize: "1.1rem", color: "var(--soft)" }}>Crafting your viral hooks...</div>
              <div style={{ fontSize: ".8rem", color: "var(--muted)", marginTop: ".5rem" }}>Goal: {goal} · Platform: {platforms.join(", ")}</div>
            </div>
          )}

          {!loading && hooks.length > 0 && (
            <>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "2rem 0 1.25rem", flexWrap: "wrap", gap: ".75rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <span style={{ fontFamily: "var(--fd)", fontWeight: 700, fontSize: "1.1rem" }}>Your Hooks</span>
                  <span style={{ padding: "3px 10px", background: "rgba(108,58,255,.12)", border: "1px solid rgba(108,58,255,.2)", borderRadius: "100px", fontSize: ".72rem", color: "#9B8CFF", fontFamily: "var(--fd)", fontWeight: 700 }}>8 generated</span>
                </div>
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  <SmBtn onClick={async () => { await navigator.clipboard.writeText(hooks.map(h => h.text).join("\n\n")); setCopied("all"); setTimeout(() => setCopied(null), 1500); }}>{copied === "all" ? "✓ Copied!" : "⎘ Copy all"}</SmBtn>
                  <SmBtn onClick={() => { const b = new Blob([hooks.map((h, i) => `${i + 1}. [${h.formula}] ${h.text}`).join("\n")], { type: "text/plain" }); const a = document.createElement("a"); a.href = URL.createObjectURL(b); a.download = "hooks.txt"; a.click(); }}>↓ Export</SmBtn>
                  <SmBtn onClick={generate}>↺ Regenerate</SmBtn>
                </div>
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                {hooks.map((h, i) => (
                  <div key={h.id} onClick={() => copyText(h.text, h.id)}
                    style={{ background: "var(--s1)", border: "1px solid var(--border)", borderRadius: "var(--r3)", padding: "1.4rem", cursor: "pointer", position: "relative", transition: "all .35s cubic-bezier(.16,1,.3,1)", animation: `cardIn .5s cubic-bezier(.16,1,.3,1) ${i * 0.05}s both` }}
                    onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.transform = "translateY(-5px)"; el.style.borderColor = "rgba(108,58,255,.45)"; el.style.boxShadow = "0 20px 50px rgba(108,58,255,.15)"; }}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1rem" }}>
                      <span style={{ fontSize: ".68rem", fontFamily: "var(--fd)", fontWeight: 700, letterSpacing: ".5px", padding: "3px 10px", borderRadius: "100px", background: "rgba(108,58,255,.1)", color: "#9B8CFF", border: "1px solid rgba(108,58,255,.2)" }}>{h.formula}</span>
                      <div style={{ display: "flex", gap: "5px" }}>
                        <button onClick={e => { e.stopPropagation(); toggleFav(h); }} style={{ width: "26px", height: "26px", borderRadius: "50%", border: `1px solid ${favorites.includes(h.id) ? "rgba(255,184,0,.4)" : "var(--border2)"}`, background: favorites.includes(h.id) ? "rgba(255,184,0,.1)" : "var(--s2)", color: favorites.includes(h.id) ? "var(--gold)" : "var(--muted)", fontSize: "11px", cursor: "pointer", transition: "all .2s" }}>★</button>
                        <button onClick={e => { e.stopPropagation(); generateScript(h); }} style={{ width: "26px", height: "26px", borderRadius: "50%", border: "1px solid var(--border2)", background: "var(--s2)", color: "var(--soft)", fontSize: "10px", cursor: "pointer", transition: "all .2s" }} title="Generate script">▶</button>
                      </div>
                    </div>
                    <p style={{ fontSize: ".9rem", lineHeight: 1.75, color: "var(--soft)", marginBottom: "1rem" }}>{h.text}</p>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
                      <span style={{ fontSize: ".68rem", padding: "3px 9px", borderRadius: "6px", background: "rgba(108,58,255,.08)", color: "#9B8CFF", border: "1px solid rgba(108,58,255,.15)" }}>{h.platform}</span>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontFamily: "var(--fd)", fontSize: "1.4rem", fontWeight: 700, letterSpacing: "-1px", color: scoreColor(h.score), lineHeight: 1 }}>{h.score}</div>
                        <div style={{ fontSize: ".6rem", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "1px" }}>/ 100</div>
                      </div>
                    </div>
                    <div style={{ height: "3px", background: "var(--s3)", borderRadius: "3px", marginTop: ".875rem", overflow: "hidden" }}>
                      <div style={{ height: "100%", borderRadius: "3px", background: "linear-gradient(90deg,var(--electric),var(--neon))", width: `${h.score}%`, transition: "width .8s cubic-bezier(.16,1,.3,1)" }} />
                    </div>
                    {copied === h.id && <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: "rgba(0,255,178,.08)", border: "1px solid rgba(0,255,178,.3)", color: "var(--neon)", padding: "8px 20px", borderRadius: "100px", fontSize: ".78rem", fontFamily: "var(--fd)", fontWeight: 700, pointerEvents: "none", zIndex: 10, whiteSpace: "nowrap" }}>COPIED ✓</div>}
                  </div>
                ))}
              </div>

              {credits <= 3 && (
                <div style={{ marginTop: "2rem", padding: "1.5rem 2rem", borderRadius: "var(--r2)", background: "linear-gradient(135deg,rgba(255,45,107,.06),rgba(108,58,255,.07))", border: "1px solid rgba(255,45,107,.2)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1.5rem", flexWrap: "wrap" }}>
                  <div>
                    <h3 style={{ fontFamily: "var(--fd)", fontWeight: 700, fontSize: "1rem", marginBottom: ".3rem" }}>{credits === 0 ? "Daily limit reached 🚀" : `Only ${credits} credit${credits !== 1 ? "s" : ""} left`}</h3>
                    <p style={{ fontSize: ".85rem", color: "var(--soft)", fontWeight: 300 }}>Pro = unlimited hooks every day.</p>
                  </div>
                  <button onClick={() => setShowModal(true)} style={{ padding: "12px 28px", borderRadius: "100px", border: "none", background: "linear-gradient(135deg,var(--hot),var(--electric))", color: "#fff", fontSize: ".875rem", fontWeight: 500, cursor: "pointer", fontFamily: "var(--fb)", whiteSpace: "nowrap" }}>Upgrade to Pro →</button>
                </div>
              )}
            </>
          )}
...
function Chip({ label, active, onClick, color }: { label: string; active: boolean; onClick: () => void; color: string }) {
  ...
  const c = map[color];
  return (
    <button onClick={onClick} style={{ padding: "6px 14px", borderRadius: "100px", border: `1px solid ${active ? c.border : "var(--border2)"}`, background: active ? c.bg : "transparent", color: active ? c.text : "var(--muted)", fontSize: ".8rem", cursor: "pointer", fontFamily: "var(--fb)", transition: "all .2s" }}>{label}</button>
  );
}

function SmBtn({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return <button onClick={onClick} style={{ padding: "7px 16px", borderRadius: "100px", border: "1px solid var(--border2)", background: "transparent", color: "var(--soft)", fontSize: ".78rem", cursor: "pointer", fontFamily: "var(--fb)" }}>{children}</button>;
}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.85)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem", backdropFilter: "blur(12px)" }} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ background: "var(--s1)", border: "1px solid var(--border2)", borderRadius: "28px", padding: "2.5rem", maxWidth: "440px", width: "100%", position: "relative", animation: "cardIn .4s cubic-bezier(.16,1,.3,1)" }}>
        ...
        <p style={{ color: "var(--soft)", fontSize: ".875rem", marginBottom: "1.75rem", fontWeight: 300, lineHeight: 1.7 }}>Unlimited hooks, script generator, history, export and advanced AI scoring.</p>
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {[{ name: "PRO", price: "$12/mo", desc: "Unlimited hooks · Script generator · History · Export · Advanced AI", hot: true }, { name: "AGENCY", price: "$49/mo", desc: "Multi-client · API · Bulk 50 hooks · White-label", hot: false }].map(p => (
            <div key={p.name} onClick={() => alert("Add your Stripe keys to activate payments!")} style={{ background: "var(--s2)", border: `1px solid ${p.hot ? "rgba(255,45,107,.35)" : "var(--border)"}`, borderRadius: "16px", padding: "1.25rem", cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: ".4rem" }}>
                <span style={{ fontSize: ".72rem", letterSpacing: "2px", textTransform: "uppercase", fontFamily: "var(--fd)", fontWeight: 700, color: p.hot ? "var(--hot)" : "var(--neon)" }}>{p.name}</span>
                <span style={{ fontFamily: "var(--fd)", fontSize: "1.3rem", fontWeight: 700 }}>{p.price}</span>
              </div>
              <div style={{ fontSize: ".78rem", color: "var(--muted)", fontWeight: 300 }}>{p.desc}</div>
            </div>
          ))}
        </div>
      </div>
